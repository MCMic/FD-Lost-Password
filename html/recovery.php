...
*/
	...
	$ldap->search( "(&(objectClass=gosaMailAccount)(mail=".$address_mail.")(uid=".$uid."))", array("dn"));
	$attrs = $ldap->fetch();
...
/*	    echo "DN : ".$dn."<br/>";*/
	    ...
	    /*	change_password ($ui->dn, $temp_password, 0, $method);*/

	    $reinit_link = "https://elledap1.ibcp.fr/fusiondirectory/recovery.php";
...
/*	    echo "link : ". $reinit_link."<br/>";*/
		...
		$smarty->assign("step3", true);
	      ...
	      $message[]= msgPool::invalid(_("Contact your administrator : check our mail serveur"));
	  }
	else
	  {
	    /* TODO a new function */
	    $message[]= msgPool::invalid(_("There is a problem with your account, check parameters !"));
	  }
      }
...
/*        echo "Temp_password : ".$temp_uniq_id."<br/>";
        ...
        $ldap->search( "(&(objectClass=gosaMailAccount)(mail=".$address_mail.")(uid=".$uid."))", array("sambaLMPassword", "sambaPwdLastSet", "dn"));
        $attrs = $ldap->fetch();
...
/*        echo "LDAP uniq : ".$ldap_uniq_id."<br/>";
	...
	echo "DN : ".$dn."<br/>";*/
        ...
        if ((strlen($ldap_uniq_id) == 40) and ($last_time_recovery >= $now) and (strcmp($uniq_id, $ldap_uniq_id) == 0))
          {
	    ...
	    else if (($MinDiffer > 0 ) and (substr($_POST['current_password'], 0, $l) == substr($_POST['new_password'], 0, $l)))
	      {
	...
	if ($_POST['address_mail'] == "")
	...
	else
	  {
	    ...
	    $uids = get_sub_list("(&(objectClass=gosaMailAccount)(mail=".$address_mail."))", "", array(get_ou("people")), $config->current['BASE'], array("uid"), GL_SUBSEARCH | GL_NO_ACL_CHECK);
	    /*print_r($uids); */
...
/*        echo "Temp_password : ".$temp_uniq_id."<br/>";
	...
	/* Retrieve hash from the ldap */
        ...
        $ldap->search( "(&(objectClass=gosaMailAccount)(mail=".$address_mail.")(uid=".$uid."))", array("sambaLMPassword", "sambaPwdLastSet"));
        $attrs = $ldap->fetch();
...
if (($step == 2) or ($step == 4))
  { 
    foreach (array('uid', 'method', 'directory', 'address_mail', 'uniq') as $index)
      {
